...
def main_cli():
    if "yardım" in args or "-y" in args or "help" in args or "-h" in args:
        ...
        exit()
    elif os.path.isfile(solus) and "önbellektemizle" in args or "clearcache" in args:
        clear_cache_app()
        exit()
    elif not os.path.isfile(solus) and "önbellekpakettemizle" in args or "clearpackagecache" in args:
        fixer()
        exit()
    elif os.path.isfile(debian) and "hataçözücü" in args in "fixerrs" in args:
        fixer()
        exit()
    else:
        main_gui()
